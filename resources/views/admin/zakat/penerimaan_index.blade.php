            ...
            <tbody id="tabelBody">
                ...
                <tr>
                    ...
                    <td style="text-align:center;">
                        <form id="hapus-penerimaan-{{ $item->id }}" action="{{ route('zakat.penerimaan.delete', $item->id) }}" method="POST" style="display:inline;">@csrf @method('DELETE')
                            <button type="button" onclick="hapus('hapus-penerimaan-{{ $item->id }}')" style="border:none;background:none;cursor:pointer;"><i class="fa fa-trash" style="color:red;"></i></button>
                        </form>
                    </td>
                    <td style="text-align:center;"><a href="{{ route('zakat.penerimaan.edit', $item->id) }}"><i class="fa fa-edit" style="color:blue;"></i></a></td>
                </tr>
                @empty
                <tr><td colspan="10" style="text-align:center;padding:2.5rem;color:#999;"><i class="fa fa-arrow-down" style="font-size:26px;display:block;margin-bottom:8px;color:#ccc;"></i>Belum ada penerimaan zakat</td></tr>
                @endforelse
            </tbody>
        </table>
    </div>
...
@endif
<script>
function cariData(){const q=document.getElementById('cariInput').value.toLowerCase();const rows=document.querySelectorAll('#tabelBody tr');let v=0;rows.forEach(r=>{if(r.textContent.toLowerCase().includes(q)){r.style.display='';v++;}else r.style.display='none';});document.getElementById('jmlBadge').textContent=v+' transaksi';}
